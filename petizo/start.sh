...
if [ ! -f "$INSTALL_MARKER" ]; then
  ...
  python3 -m pip install --break-system-packages --target="$PYTHON_PACKAGES" --no-deps \
    filelock fsspec jinja2 MarkupSafe sympy typing-extensions networkx mpmath packaging
  ...
  python3 -m pip install --break-system-packages --target="$PYTHON_PACKAGES" --no-deps \
    scipy scikit-image lazy_loader tifffile python-bidi PyYAML Shapely pyclipper ninja
